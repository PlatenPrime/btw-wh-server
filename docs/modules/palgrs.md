...
- **PATCH `/api/pallet-groups/reorder`**  
  ...
  В теле передаётся массив пар «идентификатор группы — новый порядок». Обработка выполняется одной транзакцией. В ответе возвращается количество групп, у которых изменился порядок.  
  Доступ: роль ADMIN.
