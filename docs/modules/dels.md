...
Поставка представляет собой документ с названием, датами создания и обновления, а также объектом `artikuls`: ключи — строки-артикулы (как в моделях Art, Def, Pos), значения — числа (например количество с сайта). Поставка позволяет группировать артикулы под одним названием и хранить актуальные значения, обновляемые из sharik.ua.
