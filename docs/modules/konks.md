...
Модуль `konks` решает задачи учёта конкурентов. Он обеспечивает хранение справочника конкурентов с ключом-идентификатором (name), названием, ссылкой на сайт, ссылкой на изображение и календарём дней переучёта (`recountDays`), а также CRUD-операции для управления этим справочником.

## Сущности модуля

### Konk (Конкурент)

Конкурент — справочная сущность, представляющая компанию-конкурента. Документ содержит поле `name` — уникальный строковый ключ одним словом (slug), по которому конкурента можно однозначно найти; `title` — полное название; `url` — ссылка на сайт конкурента; `imageUrl` — ссылка на изображение (логотип или иное); `recountDays` — массив UTC-дат формата `YYYY-MM-DD`, в которые у конкурента фиксировался переучёт остатков. Идентификатор документа генерируется MongoDB (`_id`). Документ имеет стандартные временные метки создания и обновления.
...
Эндпоинт updateKonkById принимает в теле запроса только те поля, которые нужно изменить (`name`, `title`, `url`, `imageUrl`, `recountDays` — все опциональны). Обновляются лишь переданные поля; непереданные остаются без изменений. Очистка календаря дней переучёта выполняется тем же частичным обновлением: передаётся пустой массив `recountDays`.

### Дни переучёта (recountDays)

Поле `recountDays` хранит бизнес-календарь дат, которые должны исключать влияние разовых корректировок остатков на метрики продаж конкурента. Эти даты не изменяют исходные срезы в БД, но используются в расчётных модулях и экспорте отчётов: продажи конкурента за день переучёта принудительно считаются равными нулю.
...
- Body: `{ name: string, title: string, url: string, imageUrl: string, recountDays?: string[] }` (name — одним словом, без пробелов; элементы `recountDays` — даты `YYYY-MM-DD`)
...
- Path параметры: `id` (MongoDB ObjectId)
- Body: опционально `{ name?: string, title?: string, url?: string, imageUrl?: string, recountDays?: string[] }`
...
- `recountDays`: string[] — UTC-даты `YYYY-MM-DD` с днями переучёта
- `createdAt`: Date
- `updatedAt`: Date
