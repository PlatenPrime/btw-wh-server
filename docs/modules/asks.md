...
Получение всех позиций для снятия по всем активным заявкам. Каждая позиция содержит информацию о заявке (`askId`, `askArtikul`), что позволяет отслеживать, для какой заявки предназначена позиция. После отправки ответа клиенту в фоне автоматически завершаются готовые заявки.
