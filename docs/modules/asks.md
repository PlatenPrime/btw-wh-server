...
Получение заявок по артикулу.

**Запрос:**

- Query параметры: `artikul` (string, обязательное поле)

**Ответ:**

- 200: `{ message: string, data: Array<Ask>, artikul: string, count: number, ...statistics }`
- 400: `{ message: string, errors?: array }` - ошибка валидации
- 500: `{ message: string, error?: string }` - ошибка сервера
...
Получение всех позиций для снятия по всем активным заявкам. Каждая позиция содержит информацию о заявке (`askId`, `askArtikul`), зону артикула (`artZone`), остаток на паллете (`quant`) и план снятия (`plannedQuant`). После отправки ответа клиенту в фоне автоматически завершаются готовые заявки.
...
- 200: `{ message: string, data: { positionsBySector: Array<PositionsBySector> } }`
- 500: `{ message: string, error?: string }` - ошибка сервера

**Доступ:** Требует роль USER

**Примечание:** После отправки ответа клиенту в фоновом режиме автоматически проверяются заявки со статусом "processing" и завершаются те, у которых количество снятого товара уже удовлетворяет запрашиваемое количество. При автоматическом завершении создаются события и отправляются уведомления создателям заявок через Telegram.
...
```typescript
{
  // Все поля из Pos (включая quant — количество на паллете)
  ...
  artZone: string | null; // Зона хранения артикула из справочника Arts (null если Art не найден)
}
```

Поля `quant` (сколько есть на позиции паллеты) и `plannedQuant` (сколько снять) позволяют сразу показать остаток и план снятия без дополнительных запросов. `artZone` берётся из `Art.zone` по артикулу заявки и не связана с опциональным `Ask.zone`.
...
```typescript
{
  positionsBySector: Array<PositionsBySector>; // Позиции для снятия, сгруппированные по секторам паллет. Каждая позиция содержит полную информацию о заявке (askId, askArtikul, askQuant, askRemainingQuantity), зону артикула (artZone) и остаток на паллете (quant)
}
```
