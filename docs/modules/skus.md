...
- `GET /api/skus/konk/:konkName/new-since-excel?since=YYYY-MM-DD` — Excel новинок конкурента с даты; вместо ключа конкурента в пути можно указать **`all`** — тогда новинки по всем конкурентам (в колонке **«Конкурент (назва)»** — `Konk.title`)
- `GET /api/skus/konk/:konkName/invalid-excel` — Excel SKU с `isInvalid`; путь **`all`** — все невалидные по всем конкурентам (та же колонка с названием конкурента)
