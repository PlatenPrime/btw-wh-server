...
После срезов всех конкурентов крон отдельно пересматривает Perfect: одним запросом поднимает документы среза за день ключа и два предыдущих, в памяти ищет кратную инверсию `stock`/`price` (произведение почти константа, целочисленный фактор ≥ 2 в противоположные стороны). Это сбой единицы (фасовка vs штука), не продажа — аномальную точку рескейлят к масштабу соседа и пишут обратно в `data`. Скачок только цены при остатке в пределах ±10% попадает в отчёт без правки. Неоднозначные серии без возврата к одному масштабу не трогают. Ошибка review не откатывает уже записанные срезы. Разовый прогон за произвольный период — скрипт `src/modules/sku-slices/scripts/runPerfectPackFlipReview.ts` (без флага apply только отчёт).
