...
- **Экспорт Excel сравнения продаж и выручки (группа аналогов):** по паре `konk` + `prod` (query) и периоду `dateFrom`, `dateTo` формируется один файл .xlsx с блоками по 6 строк на каждый аналог (продажи/ціна/виручка аналога и Btrade), колонками «Всього» и четырьмя дельтами; внизу — итоговая секция ключ–значение. Поддерживаются опциональные параметры `abc` (фильтр по букве ABC) и `sortBy=abc` (сортировка по числовой части ABC). Логика колонок дельт аналогична отчёту по остаткам: разница в штуках/процентах для продаж и в гривнах/процентах для выручки; значения записываются в первую строку блока и отображаются в объединённых ячейках.
