...
- **Групповой Excel-отчёт по конкуренту и производителю:** по паре `konk` + `prod` (query) и периоду `dateFrom`, `dateTo` (query YYYY-MM-DD) формируется один файл .xlsx, в котором для каждого аналога этой группы строится такой же 4-строчный блок (остатки/цены аналога и Btrade), как в одиночном отчёте. Блоки идут подряд; по умолчанию порядок по артикулу; при `sortBy=abc` (можно без `abc`) — по букве ABC (A→B→C→D), затем по числовой части ABC. Отчёт предназначен для аналитики конкурента по бренду/производителю.
...
- **Экспорт Excel сравнения продаж и выручки (группа аналогов):** по паре `konk` + `prod` (query) и периоду `dateFrom`, `dateTo` формируется один файл .xlsx с блоками по 6 строк на каждый аналог (продажи/ціна/виручка аналога и Btrade), колонками «Всього» и четырьмя дельтами; внизу — итоговая секция ключ–значение. Поддерживаются опциональные параметры `abc` (фильтр по букве ABC) и `sortBy=abc` (можно без `abc`; сортировка по букве ABC A→B→C→D, внутри буквы по числовой части). Логика колонок дельт аналогична отчёту по остаткам: разница в штуках/процентах для продаж и в гривнах/процентах для выручки; значения записываются в первую строку блока и отображаются в объединённых ячейках.
...
- **GET `/api/analog-slices/konk-btrade/comparison-excel`** — экспорт Excel с сравнением срезов Btrade и конкурента сразу по группе аналогов, отфильтрованных по `konk` и `prod` за период (query: `konk`, `prod`, `dateFrom`, `dateTo`; опционально `abc`, `sortBy`). Ответ: файл .xlsx. Доступ: см. [API](../api/analog-slices.md).
...
- **GET `/api/analog-slices/konk-btrade/sales-comparison-excel`** — экспорт Excel с сравнением продаж и выручки по группе аналогов (query: `konk`, `prod`, `dateFrom`, `dateTo`; опционально `abc`, `sortBy`). Ответ: файл .xlsx. Доступ: см. [API](../api/analog-slices.md).
