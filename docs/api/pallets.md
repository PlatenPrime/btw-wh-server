...
Модуль управления паллетами. Просмотр списка, пустых паллет, по id, по title, по rowId — USER; создание, обновление, перенос позиций, удаление позиций — ADMIN; удаление паллеты — PRIME. В ответах, где паллеты отдаются в кратком виде (PalletShortDto), присутствуют опциональные поля группы паллеты: `palgrId`, `palgrTitle`. Подробнее: [../modules/pallets.md](../modules/pallets.md).
