...
Получение остатка и цены товара по аналогу. По id загружается аналог из коллекции; по полю `konkName` выбирается утилита скрапинга (air, balun, yumi, yumin, sharte, svbum), которая запрашивает данные по `url` аналога. Ответ всегда в формате `{ stock, price }`.
