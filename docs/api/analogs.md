...
Получение остатка и цены товара по аналогу. По id загружается аналог из коллекции; по полю `konkName` выбирается утилита скрапинга (balun, yumi, yumin, sharte), которая запрашивает данные по `url` аналога. **Air не поддерживается** серверным live-опросом. Ответ всегда в формате `{ stock, price }`.
