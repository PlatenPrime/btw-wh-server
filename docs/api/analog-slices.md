...
Для маршрутов продаж конкурента (`sales-by-date`, `sales-range`, `konk-btrade/sales-comparison`, `*sales-comparison-excel`) применяется календарь `Konk.recountDays`: на дату переучёта продажи и выручка конкурента принудительно считаются `0`, при этом дата не исключается из периода.

В Excel-отчётах **comparison-excel** и **sales-comparison-excel** заголовки колонок по календарным дням оформлены так же, как в модуле sku-slices: **украинское название дня недели, запятая, пробел и дата `YYYY-MM-DD`** (UTC-день).
...
- `sortBy`: string (опционально) — значение `"abc"`; можно передавать независимо от `abc`. При `sortBy=abc` блоки сортируются: сначала по букве ABC (A → B → C → D), внутри каждой буквы — по числовой части поля ABC (напр. 12A, 56A, 2B, 7B, 42B, 34C, 87C, 123C, 4D, 278D), затем по артикулу. По умолчанию (без `sortBy`) — сортировка по артикулу.
...
**Запрос:** только query: `konk`, `prod` (string, обязательно), `dateFrom`, `dateTo` (string, YYYY-MM-DD, обязательно), dateTo ≥ dateFrom; опционально `abc` (одна из букв A, B, C, D — фильтр по полю ABC артикула без учёта регистра), `sortBy` (значение `"abc"` — можно передавать с или без `abc`; при `sortBy=abc` блоки сортируются по букве ABC A→B→C→D, внутри буквы по числовой части ABC, затем по артикулу; без `sortBy` — по артикулу). Итоговая секция в Excel считается по отфильтрованным и отсортированным данным.
