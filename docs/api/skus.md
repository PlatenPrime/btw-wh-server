...
**Ответ 200:** бинарный поток `.xlsx`, заголовки `Content-Type` и `Content-Disposition` как у Excel в `sku-slices`. В колонке **«Дата створення»** значение даты — строка вида **украинский день недели, запятая, пробел, `YYYY-MM-DD`** (UTC-день как у срезов).
...
**Ответ 200:** `.xlsx` (может быть пустой таблица заголовков, если таких SKU нет). Формат колонки **«Дата створення»** — как у `new-since-excel`.
