...
Эндпоинты чтения ежедневных срезов остатков и цен по SKU конкурентов. Для большинства маршрутов данные Btrade не используются; исключение — **konk-prod chart data** (сравнение агрегата SKU с Btrade по производителю). Доступ: checkAuth + checkRoles(USER), кроме случаев, если в коде роутера указано иное.
...
**Ошибки:** 400, 401, 403, 404 (нет SKU с `productId` для пары konk/prod), 500.

**Примечание:** если ни у одного **Art** нет `prodName`, совпадающего с query `prod`, серия Btrade в `days` — нули (в т.ч. при наличии строк в `BtradeSlice` по артикулам без подходящего Art); ответ остаётся 200.
