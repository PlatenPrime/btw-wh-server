...
Excel-файл продаж по всем SKU конкурента за период. Для каждого SKU строится блок из трех строк: `Продажі`, `Ціна`, `Виручка`.

Внизу листа добавляется общий итог по всем товарам конкурента:
- `Загальні продажі, шт`
- `Загальна виручка, грн`

Файл не содержит сравнения с Btrade.

**Query:** `konk` (string, обязательно), `dateFrom`, `dateTo` (YYYY-MM-DD, обязательно).

**Ответ 200:** бинарный XLSX; имя файла вида `sku_sales_konk_<konk>_<dateFrom>_<dateTo>.xlsx`.
