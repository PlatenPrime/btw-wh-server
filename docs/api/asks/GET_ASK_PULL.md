...
- Остатках в позициях с таким же артикулом
...
### Сценарий 3: Позиций с таким артикулом нет
...
1. Позиции автоматически сортируются по сектору паллеты (по возрастанию)
2. Если у паллеты нет сектора (`sector === null/undefined`), она считается как сектор 0
3. Распределение количества идет последовательно по отсортированным позициям до покрытия всего `remainingQuantity`
4. Если доступного количества на всех позициях недостаточно, возвращаются все доступные позиции с их остатками
5. Поле `plannedQuant` показывает, сколько нужно снять с конкретной позиции
6. Поле `quant` показывает текущее количество на позиции
7. Флаг `isPullRequired` позволяет фронтенду понять, нужно ли показывать интерфейс для снятия товара
