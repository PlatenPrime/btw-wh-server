...
data.positionsBySector.forEach((sectorGroup) => {
  ...
  sectorGroup.positions.forEach((position) => {
    console.log(
      `  - ${position.artikul} (ask: ${position.askId}): ${position.plannedQuant} шт.`
    );
  });
});
...
- **Информация о заявках**: Каждая позиция содержит поля `askId` и `askArtikul`, что позволяет отслеживать, для какой заявки предназначена позиция. Это особенно важно, когда есть несколько заявок с одинаковым артикулом - позиции для разных заявок можно легко различить
