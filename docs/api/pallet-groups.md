...
#### Перестановка порядка групп одним запросом

**PATCH `/api/pallet-groups/reorder`**  
Обновляет порядок нескольких групп паллет одним запросом в одной транзакции.

- **Назначение для фронта**:  
  использовать при изменении порядка групп (например, перетаскивание колонок) вместо множества вызовов PUT по одной группе — один запрос, одна транзакция, согласованное состояние.
- **Тело запроса** ожидает объект с полем `orders` — массивом пар «идентификатор группы — новый порядок»:
  - `orders`: массив объектов `{ "id": "<ObjectId группы>", "order": <целое ≥ 1> }`;
  - минимум один элемент;
  - все `id` должны быть разными, все значения `order` в массиве — разные.
- **Ответ** при успехе (200):
  - `message` — строка об успешном выполнении;
  - `data.updatedCount` — количество групп, у которых фактически изменился порядок.
- **Ошибки**:
  - 400: невалидное тело (формат, дубликаты id или order), отсутствующая группа по переданному id.
...
#### PATCH `/api/pallet-groups/reorder` — перестановка порядка групп

**Запрос:**

```http
PATCH /api/pallet-groups/reorder HTTP/1.1
Host: example.com
Authorization: Bearer <token>
Content-Type: application/json
...
**Ответ 400 (невалидные данные или группа не найдена):**

```json
{
  "message": "Pallet group not found: 507f1f77bcf86cd799439099"
}
```
