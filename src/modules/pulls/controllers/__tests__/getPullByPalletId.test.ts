...
describe("getPullByPalletId Controller", () => {
  ...
  it("should return pull for valid pallet ID", async () => {
    ...
    const { calculatePullByPalletId } = await import(
      "../../utils/calculatePulls.js"
    );
    vi.mocked(calculatePullByPalletId).mockResolvedValue(mockPull as any);
    ...
    // Act
    await getPullByPalletId(mockRequest as Request, res);
    ...
    expect(calculatePullByPalletId).toHaveBeenCalledWith(
      new mongoose.Types.ObjectId(palletId)
    );
  });
  ...
  it("should return 404 when pull not found", async () => {
    ...
    expect(responseJson.message).toBe(
      "Pull not found for the specified pallet"
    );
  ...
  it("should handle calculation errors", async () => {
    ...
    vi.mocked(calculatePullByPalletId).mockRejectedValue(
