...
describe("processPullPosition Controller", () => {
  ...
  it("should process position successfully with partial quantity", async () => {
    ...
    vi.mocked(Pos.findById).mockResolvedValue(testPosition as any);
    vi.mocked(User.findById).mockResolvedValue(solverUser as any);
    vi.mocked(Ask.find).mockResolvedValue([testAsk] as any);
    ...
    // Act
    await processPullPosition(mockRequest as Request, res);
  ...
  it("should remove position when quantity becomes 0", async () => {
    ...
    vi.mocked(User.findById).mockResolvedValue(solverUser as any);
    vi.mocked(Ask.find).mockResolvedValue([testAsk] as any);
    vi.mocked(Pos.findByIdAndDelete).mockResolvedValue(testPosition as any);
    vi.mocked(Pallet.findByIdAndUpdate).mockResolvedValue({} as any);
    vi.mocked(Ask.findByIdAndUpdate).mockResolvedValue({
      ...testAsk,
      actions: [
        ...testAsk.actions,
    ...
    expect(Pos.findByIdAndDelete).toHaveBeenCalledWith(
      posId,
      expect.any(Object)
    );
    expect(Pallet.findByIdAndUpdate).toHaveBeenCalledWith(
      palletId,
      { $pull: { poses: posId } },
      expect.any(Object)
    );
  ...
  it("should return 400 for invalid pallet ID format", async () => {
    ...
    expect(responseJson.message).toBe(
      "Invalid pallet ID or position ID format"
    );
  });
  ...
  it("should return 400 when actual quantity exceeds available quantity", async () => {
    ...
    expect(responseJson.success).toBe(false);
    expect(responseJson.message).toBe(
      "Actual quantity cannot exceed available quantity"
    );
  });
  ...
  it("should handle database transaction errors", async () => {
    ...
    vi.mocked(Pos.findById).mockRejectedValue(
