...
import type { GetSkugrSkusSalesInput } from "../schemas/getSkugrSkusSalesSchema.js";

export type SkugrSkuSalesRow = {
  skuId: string;
  title: string;
  productId: string;
  imageUrl: string;
...
export async function getSkugrSkusSalesUtil(
  input: GetSkugrSkusSalesInput,
): Promise<GetSkugrSkusSalesResult> {
  ...
  const metrics = await aggregatePeriodSkuSliceMetricsForSkus(
  ...
  const data: SkugrSkuSalesRow[] = skus.map((s, i) => {
