...
describe("coalesceBtradeSliceItemsForReporting", () => {
  ...
  it("forward-fills quantity and price skipping -1", () => {
    ...
    const out = coalesceBtradeSliceItemsAlongDates([d1, d2, d3], (d) => {
      const t = d.getTime();
      if (t === d1.getTime()) return { quantity: 10, price: 100 };
      if (t === d2.getTime()) return { quantity: -1, price: -1 };
      if (t === d3.getTime()) return { quantity: 8, price: 120 };
      return undefined;
    });
