...
export interface SkuStockResult {
  stock: number;
  price: number;
}
...
export async function getSkuStockDataUtil(
  ...
  if (!getter) {
    const err = new Error(
      `Unsupported competitor for stock: ${sku.konkName}`
  ...
  const price =
    result.price !== undefined && result.price !== null ? result.price : -1;
