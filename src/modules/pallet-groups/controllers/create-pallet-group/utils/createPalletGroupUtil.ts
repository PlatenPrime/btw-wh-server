...
type CreatePalletGroupUtilInput = {
  title: string;
  order?: number;
  session: ClientSession;
};

export const createPalletGroupUtil = async ({
  title,
  order,
  session,
}: CreatePalletGroupUtilInput): Promise<IPalletGroup> => {
  const existingWithTitle = await PalletGroup.findOne({ title }).session(
    session,
  );
  ...
  const targetOrder =
    order && Number.isInteger(order) && order > 0 && order <= totalGroups + 1
      ? order
      : totalGroups + 1;
  ...
  return createdGroup[0] as IPalletGroup;
};
