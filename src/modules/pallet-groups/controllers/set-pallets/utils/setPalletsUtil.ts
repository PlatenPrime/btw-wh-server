...
export const setPalletsUtil = async ({
  groupId,
  palletIds,
  session,
}: SetPalletsUtilInput): Promise<IPalletGroup> => {
  ...
  const palletObjectIds = palletIds.map(
  ...
  const removedPalletIds = previousPalletIds.filter(
    (id) => !palletIds.includes(id),
  );
