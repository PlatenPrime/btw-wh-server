...
import type { IDeficitCalculationResult } from "../../../../types.js";
...
describe("enrichDefsWithAsksUtil", () => {
  ...
  it("обогащает дефициты информацией о заявках", async () => {
    const resultInput: IDeficitCalculationResult = {
      ART001: {
        nameukr: "Товар 1",
        quant: 10,
        sharikQuant: 5,
  ...
  it("обрабатывает случай когда заявок нет", async () => {
    ...
    (Ask as any).find = vi.fn().mockReturnValue({
      select: vi.fn().mockReturnValue({
    ...
    const mockAsks = [
      {
        ...
        status: "new",
        createdAt: new Date("2024-01-10T10:00:00.000Z"),
        askerData: { fullname: "Иван Иванов", _id: "user-id-1" as any },
      },
      {
        _id: "ask-id-2" as any,
        artikul: "ART001",
        ...
        askerData: { fullname: "Петр Петров", _id: "user-id-2" as any },
      },
    ];
