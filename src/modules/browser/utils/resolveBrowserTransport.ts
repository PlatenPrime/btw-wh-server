...
export function parseBrowserTransportByKonk(
...
): Map<string, BrowserTransport> {
  const map = new Map<string, BrowserTransport>();
  ...
  for (const part of trimmed.split(",")) {
    ...
    if (colonIdx <= 0) {
      browserLog.warn(
        { entry },
        "BROWSER_TRANSPORT_BY_KONK: skip malformed entry"
      );
      continue;
    }
    ...
    if (!konk) {
      browserLog.warn(
        { entry },
        "BROWSER_TRANSPORT_BY_KONK: skip empty konkName"
      );
      continue;
    }
    if (!VALID_TRANSPORTS.has(transportRaw as BrowserTransport)) {
      browserLog.warn(
        { entry, transport: transportRaw },
        "BROWSER_TRANSPORT_BY_KONK: invalid transport, ignored"
      );
      continue;
    }
    map.set(konk, transportRaw as BrowserTransport);
  }
...
export function resolveBrowserTransport(
  ...
  const map = parseBrowserTransportByKonk(
    process.env.BROWSER_TRANSPORT_BY_KONK
  );
