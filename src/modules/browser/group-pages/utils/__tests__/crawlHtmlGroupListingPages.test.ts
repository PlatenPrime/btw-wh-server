...
function htmlWithNext(body: string, nextHref?: string): string {
  const next =
    nextHref != null
...
describe("getNextPageUrlFromLinkRelNext", () => {
  it("returns null when rel=next is absent", () => {
    ...
    expect(
      getNextPageUrlFromLinkRelNext($, START, () => null)
    ).toBeNull();
  });

  it("returns null when resolveUrl rejects the href", () => {
    const $ = cheerio.load(
      '<html><head><link rel="next" href="/p2" /></head></html>'
    );
    expect(
      getNextPageUrlFromLinkRelNext($, START, () => null)
    ).toBeNull();
  ...
  it("resolves next href via resolveUrl", () => {
    const $ = cheerio.load(
      '<html><head><link rel="next" href="page2" /></head></html>'
    );
    const base = "https://example.test/list/";
    const resolved = getNextPageUrlFromLinkRelNext(
      $,
      base,
...
describe("crawlHtmlGroupListingPages", () => {
  ...
  it("merges products across two pages", async () => {
    ...
    vi.mocked(browserGet).mockImplementation(async (url: string) => {
      if (url === START) return h1;
      if (url === PAGE2) return h2;
    ...
    const result = await crawlHtmlGroupListingPages({
      ...
      parseProductsFromPage: ($, pageUrl) => {
        const m = new Map<string, { id: string; from: string }>();
        $("span[data-id]").each((_, el) => {
          const id = $(el).attr("data-id");
          if (id) m.set(id, { id, from: pageUrl });
        });
        return m;
      },
  ...
  it("stops when stopOnEmptyPage and parser returns empty map", async () => {
    ...
    const result = await crawlHtmlGroupListingPages({
      ...
      maxPages: 10,
      parseProductsFromPage: ($) => {
        const m = new Map<string, { k: string }>();
        $("b[data-k]").each((_, el) => {
  ...
  it("stops when next URL was already visited", async () => {
    ...
    const result = await crawlHtmlGroupListingPages({
      ...
      parseProductsFromPage: ($) => {
        const m = new Map<string, { k: string }>();
        if ($("b[data-k]").length) m.set("x", { k: "x" });
        return m;
      },
      getNextPageUrl: ($, url) => getNextPageUrlFromLinkRelNext($, url, resolve),
    });
