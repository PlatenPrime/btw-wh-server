...
export const AIR_PRODUCT_FETCH_RETRY_DELAYS_MS = [2000, 5000] as const;

const MAX_RETRY_AFTER_MS = 60_000;

type AirRequestAgents = {
  httpAgent: Agent;
  httpsAgent: Agent;
};
 ...
 * Кап 60s.
 */
export function resolveRetryAfterMs(retryAfter: unknown): number | undefined {
...
export function resolveAirProductOrigin(productUrl: string): string {
 ...
 */
export function resolveAirProxyAgents(): AirRequestAgents | undefined {
...
function formatAirFetchHttpError(url: string, status: number): string {
  return `Browser GET HTTP ${status}: ${url}`;
}

type WarmUpResult = { cookieHeader: string };

async function warmUpAirOrigin(
  origin: string,
  agents: AirRequestAgents | undefined
): Promise<WarmUpResult> {
  const client = getBrowserAxios();
  const warmUrl = `${origin}/`;
  const response = await client.get<string>(warmUrl, {
    ...AIR_TEXT_CONFIG,
 ...
 */
export async function fetchAirProductHtml(productUrl: string): Promise<string> {
  ...
  for (let attempt = 0; attempt < AIR_PRODUCT_FETCH_MAX_ATTEMPTS; attempt++) {
    ...
    const response = await client.get<string>(trimmed, {
      ...AIR_TEXT_CONFIG,
    ...
    const respHeaders = (response.headers ?? {}) as Record<string, unknown>;

    cookieHeader = mergeCookies(
      cookieHeader,
      pickHeaderCaseInsensitive(respHeaders, "set-cookie")
    );
    ...
    if (status === 429 && attempt < AIR_PRODUCT_FETCH_MAX_ATTEMPTS - 1) {
      const retryAfterMs = resolveRetryAfterMs(
        pickHeaderCaseInsensitive(respHeaders, "retry-after")
      );
      const fallbackMs =
        AIR_PRODUCT_FETCH_RETRY_DELAYS_MS[attempt] ??
        AIR_PRODUCT_FETCH_RETRY_DELAYS_MS[
          AIR_PRODUCT_FETCH_RETRY_DELAYS_MS.length - 1
        ];
