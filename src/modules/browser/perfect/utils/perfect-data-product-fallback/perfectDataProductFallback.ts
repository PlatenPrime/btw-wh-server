...
type DataProductRecord = Record<string, unknown>;

function parseDataProductAttr(raw: string | undefined): DataProductRecord | null {
  if (!raw?.trim()) return null;
  try {
    const parsed: unknown = JSON.parse(raw);
    if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
      return parsed as DataProductRecord;
    }
  } catch {
    return null;
  }
...
function decodeDataProductAttrValue(encoded: string): string {
  return encoded
    .replace(/&quot;/g, '"')
    .replace(/&amp;/g, "&")
    .replace(/&#039;/g, "'");
...
export function extractDataProductFromHtml(html: string): DataProductRecord | null {
...
export function extractDataProductFieldsFromHtmlRegex(html: string): DataProductRecord | null {
  ...
  const record: DataProductRecord = {};
  if (quantityMatch?.[1]) record.quantity = quantityMatch[1];
  if (priceAmountMatch?.[1]) record.price_amount = priceAmountMatch[1];
  else if (priceTaxExcMatch?.[1]) record.price_tax_exc = priceTaxExcMatch[1];
  if (nameMatch?.[1]) {
    try {
      record.name = JSON.parse(`"${nameMatch[1]}"`) as string;
    } catch {
...
export function tryPerfectDataProductFallback(
  html: string,
  pageTitle: string
  ...
  let packPrice =
    parseNumberLike(dataProduct.price_amount) ??
  ...
  if (stockPacksRaw === null || packPrice === null) return null;
