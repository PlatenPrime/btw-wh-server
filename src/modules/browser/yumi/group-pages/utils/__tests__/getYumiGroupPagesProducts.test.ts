...
function splitDump(dump: string): { dumpUrl: string; html: string } {
...
describe("getYumiGroupPagesProducts", () => {
  ...
  it("should parse products from page_1 and page_2", async () => {
    vi.mocked(browserGet).mockImplementation(async (url: string) => {
      if (url === page1.dumpUrl) return page1.html;
      if (url === page2.dumpUrl) return page2.html;
    ...
    expect(product?.url).toBe(
      "https://yumi-market.com.ua/ua/p928707587-gemar-dzh-g90.html"
    );
