...
export const GRABO_FETCH_RETRY_WAIT_MS = [8_000, 20_000, 45_000] as const;
 ...
 */
export async function fetchGraboPageHtml(url: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= GRABO_FETCH_MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (attempt === GRABO_FETCH_MAX_ATTEMPTS || waitMs == null) {
        log.warn(
          { url, attempt, maxAttempts: GRABO_FETCH_MAX_ATTEMPTS, details },
