...
describe("exportArtsToExcelWithStocksController", () => {
  ...
  it("200: успешный экспорт артикулов с запасами", async () => {
    ...
    const mockPosesQuantMap = new Map<string, number>();
    ...
    expect(formatArtsForExcelExtendedUtil).toHaveBeenCalledWith(
      mockArts,
      mockPosesQuantMap
    );
  ...
  it("404: артикулы null", async () => {
    vi.mocked(getArtsForExportExtendedUtil).mockResolvedValue(null as any);
