...
import { getArtsForExportUtil } from "../getArtsForExportUtil.js";

describe("getArtsForExportUtil", () => {
  beforeEach(async () => {
  ...
  it("получает все артикулы из базы данных", async () => {
    ...
    const result = await getArtsForExportUtil();
  ...
  it("возвращает только нужные поля", async () => {
    await createTestArt({
      artikul: "ART-001",
      nameukr: "Тест",
      namerus: "Тест",
    ...
    expect((art as any).save).toBeUndefined();
    expect((art as any).toObject).toBeUndefined();
  });
  ...
  it("использует lean() - возвращает plain objects", async () => {
    ...
    expect((art as any).save).toBeUndefined();
    expect((art as any).toObject).toBeUndefined();
    expect((art as any).toJSON).toBeUndefined();
    expect((art as any).isNew).toBeUndefined();
  });
