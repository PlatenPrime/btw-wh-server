...
describe("getAsksByArt", () => {
  ...
  it("200: возвращает заявки указанного артикула и статистику", async () => {
    ...
    const req = { query: { artikul: "ART-TEST" } } as unknown as Request;
    ...
    expect(responseJson.data.every((ask: any) => ask.artikul === "ART-TEST")).toBe(
      true
    );
  });
  ...
  it("400: ошибка валидации при пустом artikul", async () => {
    const req = { query: { artikul: "" } } as unknown as Request;
    await getAsksByArt(req, res);
    expect(responseStatus.code).toBe(400);
    expect(responseJson.message).toBe("Validation error");
  });
