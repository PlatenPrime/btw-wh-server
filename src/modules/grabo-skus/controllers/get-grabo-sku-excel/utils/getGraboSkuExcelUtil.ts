...
import {
  buildGraboSkuExcelBuffer,
  type GraboSkuExcelRow,
} from "../../../utils/buildGraboSkuExcel.js";

export async function getGraboSkuExcelUtil(): Promise<{
  buffer: Buffer;
  fileName: string;
}> {
  const docs = await GraboSku.find().sort({ productId: 1 }).lean().exec();
  const rows: GraboSkuExcelRow[] = docs.map((doc) => ({
