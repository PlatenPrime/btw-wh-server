...
describe("deletePalletEmptyPoses Controller", () => {
  ...
  it("should delete empty poses (quant=0, boxes=0) from a specific pallet", async () => {
    ...
    pallet.poses = [
      emptyPos1._id as Types.ObjectId,
      emptyPos2._id as Types.ObjectId,
      nonEmptyPos._id as Types.ObjectId,
    ];
    await pallet.save();

    mockRequest = {
      params: {
        id: (pallet._id as Types.ObjectId).toString(),
      },
    };

    // Act
    await deletePalletEmptyPoses(mockRequest as Request, res);

    // Assert
    expect(responseStatus.code).toBe(200);
    expect(responseJson.message).toBe(
      "Empty poses removed from pallet successfully"
    );
    expect(responseJson.deletedCount).toBe(2);
    expect(responseJson.affectedPoseIds).toHaveLength(2);
    expect(responseJson.affectedPoseIds).toContain(
      (emptyPos1._id as Types.ObjectId).toString()
    );
    expect(responseJson.affectedPoseIds).toContain(
      (emptyPos2._id as Types.ObjectId).toString()
    );
    ...
    expect(updatedPallet?.poses.map((id) => id.toString())).not.toContain(
      (emptyPos1._id as Types.ObjectId).toString()
    );
    expect(updatedPallet?.poses.map((id) => id.toString())).not.toContain(
      (emptyPos2._id as Types.ObjectId).toString()
    );
    expect(updatedPallet?.poses.map((id) => id.toString())).toContain(
      (nonEmptyPos._id as Types.ObjectId).toString()
    );
  });
  ...
  it("should handle database errors gracefully", async () => {
    ...
    vi.spyOn(Pallet, "findById").mockRejectedValueOnce(
  ...
  it("should handle transaction errors", async () => {
    ...
    vi.spyOn(Pos, "deleteMany").mockRejectedValueOnce(
