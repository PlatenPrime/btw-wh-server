...
export type DeleteSkugrWithSkusResult = {
  deletedSkusCount: number;
  modifiedSkugrsCount: number;
};

export const deleteSkugrWithSkusUtil = async (
  id: string,
): Promise<DeleteSkugrWithSkusResult | null> => {
  const target = await Skugr.findById(id).select("skus").lean();
  if (!target) {
    return null;
  }

  const skuIds = (target.skus ?? []) as Types.ObjectId[];
  ...
  try {
    ...
    await session.withTransaction(async () => {
      if (skuIds.length > 0) {
        const pullRes = await Skugr.updateMany(
          { _id: { $ne: targetOid }, skus: { $in: skuIds } },
          { $pullAll: { skus: skuIds } },
          { session },
        );
