...
/**
 * Привязывает/создаёт Sku по уже распарсенным карточкам листинга. Без сети.
 */
export async function fillSkugrSkusFromProductsUtil(
  skugr: ISkugr,
  products: GroupBrowserProduct[],
): Promise<FillSkugrSkusFromBrowserResult | null> {
  const skugrId = skugr._id.toString();
