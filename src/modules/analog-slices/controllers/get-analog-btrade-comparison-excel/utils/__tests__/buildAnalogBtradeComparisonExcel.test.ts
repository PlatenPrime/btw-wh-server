...
describe("buildAnalogBtradeComparisonExcel", () => {
  it("builds excel with correct headers and 4 data rows", async () => {
    ...
    expect(headerRow[6]).toBe(
      formatExcelDateHeaderUk(new Date("2026-03-01T00:00:00.000Z"))
    );
    expect(headerRow[7]).toBe(
      formatExcelDateHeaderUk(new Date("2026-03-02T00:00:00.000Z"))
    );
  ...
  it("marks percentage cells with pale pink fill when value cannot be calculated", async () => {
    ...
    const diffFill = diffPctCell.fill as ExcelJS.FillPattern | undefined;
    if (diffFill?.fgColor?.argb) expect(diffFill.fgColor.argb).toBe("FFFFC0CB");
