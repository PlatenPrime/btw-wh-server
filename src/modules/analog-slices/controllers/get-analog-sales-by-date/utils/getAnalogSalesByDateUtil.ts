 ...
 */
export async function getAnalogSalesByDateUtil(
  input: GetAnalogSalesByDateInput
): Promise<SalesByDateResult | null> {
  ...
  const currData = (currDoc?.data ?? {}) as Record<string, IAnalogSliceDataItem>;
  const currItem = currData[artikulKey];
  if (!currItem) return null;

  const prevData = (prevDoc?.data ?? {}) as Record<string, IAnalogSliceDataItem>;
