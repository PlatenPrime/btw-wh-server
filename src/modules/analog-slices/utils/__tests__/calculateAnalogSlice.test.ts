...
describe("calculateAnalogSlice", () => {
  ...
  it.each(ANALOG_SLICE_KONK_NAMES)(
    "delegates %s to runAnalogSliceForKonkUtil",
    async (konkName) => {
      const result = await calculateAnalogSlice(konkName);

      expect(runAnalogSliceForKonkUtil).toHaveBeenCalledWith(
        konkName,
