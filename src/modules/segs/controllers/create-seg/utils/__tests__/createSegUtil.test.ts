...
describe("createSegUtil", () => {
  const createZone = async (
    overrides: Partial<{ title: string; bar: number }> = {}
  ) => {
  ...
  it("creates segment, updates block.segs and assigns zones", async () => {
    ...
    const createdSeg = await runWithSession((session) =>
      createSegUtil({
        blockData: block,
        order: 3,
    ...
    expect(updatedBlock?.segs.map((id) => id.toString())).toContain(
      createdSeg._id.toString()
    );
  ...
  it("throws when zones already belong to segments", async () => {
    ...
    await Zone.updateOne(
      { _id: zone._id },
      { $set: { "seg.id": existingSeg._id, sector: existingSeg.sector } }
    );
