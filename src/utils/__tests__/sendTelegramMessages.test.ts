...
describe("sendTelegramMessages", () => {
  ...
  describe("sendMessageToChat", () => {
    ...
    it("should throw error for empty message", async () => {
      await expect(sendMessageToChat("", "123")).rejects.toThrow(
        "Message cannot be empty"
      );
    });

    it("should throw error for empty chatId", async () => {
      await expect(sendMessageToChat("Test", "")).rejects.toThrow(
    ...
    it("should handle API errors", async () => {
      ...
      await expect(sendMessageToChat("Test", "123")).rejects.toThrow(
        "Failed to send message to chat: API Error"
      );
    });
  });
  ...
  describe("sendMessageToUser", () => {
    ...
    it("should throw error for empty message", async () => {
      await expect(sendMessageToUser("", "123")).rejects.toThrow(
        "Message cannot be empty"
      );
    });

    it("should throw error for empty userId", async () => {
      await expect(sendMessageToUser("Test", "")).rejects.toThrow(
    ...
    it("should handle unknown errors", async () => {
      ...
      await expect(sendMessageToUser("Test", "123")).rejects.toThrow(
        "Failed to send message to user: Unknown error occurred"
      );
    });
  });
  ...
  describe("sendMessageToPlaten", () => {
    ...
    it("should propagate errors from sendMessageToUser", async () => {
      ...
      await expect(sendMessageToPlaten("Test")).rejects.toThrow(
        "Failed to send message to user: User not found"
      );
    });
  });
  ...
  describe("sendFileToUser", () => {
    ...
    it("should throw error for empty file path", async () => {
      await expect(sendFileToUser("", "123")).rejects.toThrow(
        "File path cannot be empty"
      );
    });

    it("should throw error for empty userId", async () => {
      await expect(sendFileToUser("/path/file.txt", "")).rejects.toThrow(
        "User ID cannot be empty"
      );
    });
    ...
    it("should throw error if file does not exist", async () => {
      ...
      await expect(
        sendFileToUser("/nonexistent/file.txt", "123")
      ).rejects.toThrow("File does not exist: /nonexistent/file.txt");
    });

    it("should handle API errors", async () => {
      mockedAxios.post.mockRejectedValue(new Error("File too large"));

      await expect(sendFileToUser("/path/file.txt", "123")).rejects.toThrow(
        "Failed to send file to user: File too large"
      );
    });
  ...
  describe("sendFileToPlaten", () => {
    ...
    it("should propagate errors from sendFileToUser", async () => {
      ...
      await expect(sendFileToPlaten("/path/file.txt")).rejects.toThrow(
        "Failed to send file to user: Network timeout"
      );
    });
  });
