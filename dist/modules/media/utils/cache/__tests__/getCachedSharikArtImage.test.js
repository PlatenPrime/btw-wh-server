...
import { clearSharikArtImageCache, getCachedSharikArtImage, getSharikArtImageCacheSize, } from "../getCachedSharikArtImage.js";
...
describe("getCachedSharikArtImage", () => {
    ...
    it("dedupes parallel miss into one fetch", async () => {
        let resolveFetch;
        vi.mocked(fetchSharikArtImage).mockImplementation(() => new Promise((resolve) => {
    ...
    it("evicts oldest LRU entry when max size exceeded", async () => {
        ...
        expect(getSharikArtImageCacheSize()).toBe(SHARIK_ART_IMAGE_CACHE_MAX_ENTRIES);
        vi.mocked(fetchSharikArtImage).mockResolvedValueOnce({
