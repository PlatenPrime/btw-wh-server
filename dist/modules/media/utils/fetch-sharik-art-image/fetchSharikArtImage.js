...
import { buildSharikArtImageUrl, } from "../build-sharik-art-image-url/buildSharikArtImageUrl.js";
export class SharikArtImageNotFoundError extends Error {
    code = "SHARIK_ART_IMAGE_NOT_FOUND";
...
export class SharikArtImageUpstreamError extends Error {
    code = "SHARIK_ART_IMAGE_UPSTREAM";
 ...
 */
export async function fetchSharikArtImage(artikul, size) {
    ...
    if (result.status >= 500) {
        throw new SharikArtImageUpstreamError(`Upstream HTTP ${result.status} for sharik art image`);
    }
