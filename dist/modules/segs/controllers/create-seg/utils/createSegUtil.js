...
export const createSegUtil = async ({ blockData, order, zones, session, }) => {
    ...
    const zonesWithSeg = existingZones.filter((zone) => zone.seg && zone.seg.id);
    if (zonesWithSeg.length > 0) {
        throw new Error(`Zones already belong to segments: ${zonesWithSeg.map((z) => z._id.toString()).join(", ")}`);
    }
    ...
    blockData.segs.push(created[0]._id);
