...
export const updateSegUtil = async ({ segId, updateData, session, }) => {
    ...
    if (updateData.zones !== undefined) {
        ...
        const zoneObjectIds = updateData.zones.map((zoneId) => new mongoose.Types.ObjectId(zoneId));
        ...
        const zonesWithOtherSegs = existingZones.filter((zone) => zone.seg &&
            zone.seg.id &&
    ...
    // Обновить сегмент
    const updatedSeg = await Seg.findByIdAndUpdate(objectId, updateDataSeg, { new: true, session }).exec();
    return updatedSeg;
};
