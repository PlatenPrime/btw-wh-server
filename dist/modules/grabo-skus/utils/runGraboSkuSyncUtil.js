...
async function delayIfNeeded(delayMs, isFirst) {
    if (isFirst) {
        return;
    }
    const ms = delayMs();
 ...
 */
export async function runGraboSkuSyncUtil(deps = {}) {
    ...
    for (let i = 0; i < catalog.productUrls.length; i++) {
        ...
        try {
            ...
            if (!data.productId.trim()) {
                ...
                log.info({ index, total: stats.listed, url, result: "skipped-no-productId" }, "grabo sku product");
                continue;
            }
            const result = await upsertGraboSkuUtil({ ...data, productId: data.productId.trim(), url }, now());
            if (result === "created") {
                stats.created += 1;
            }
            else {
    ...
    if (catalogComplete && catalog.productUrls.length > 0) {
        ...
        log.info({ markedOffSite: stats.markedOffSite }, "grabo sku absent-pass done");
    }
    else {
        log.info({ catalogComplete, listed: stats.listed }, "grabo sku absent-pass skipped");
    }
