...
function skuData(productId, title) {
    return {
        title,
        productId,
        isNew: false,
...
describe("runGraboSkuSyncUtil", () => {
    ...
    it("creates and updates by productId", async () => {
        ...
        const stats = await runGraboSkuSyncUtil({
            ...
            getSkuData: async (url) => url === URL_A ? skuData("GA", "New A") : skuData("GB", "B"),
            delayMs: () => 0,
        ...
        expect((await GraboSku.findOne({ productId: "GA" }).lean())?.title).toBe("New A");
    ...
    it("marks off-site only urls missing from listing when catalog is complete", async () => {
        ...
        expect((await GraboSku.findOne({ productId: "GG" }).lean())?.isOnSite).toBe(false);
    });
    it("does not absent-pass when a category crawl failed", async () => {
        ...
        expect((await GraboSku.findOne({ productId: "GG" }).lean())?.isOnSite).toBe(true);
    });
    it("does not flip isOnSite when product fetch fails", async () => {
        ...
        expect(stats.markedOffSite).toBe(0);
        expect((await GraboSku.findOne({ productId: "GA" }).lean())?.isOnSite).toBe(true);
    });
    ...
    it("jitters between product fetches, not before the first", async () => {
        ...
        await runGraboSkuSyncUtil({
            ...
            getSkuData: async (url) => url === URL_A ? skuData("GA", "A") : skuData("GB", "B"),
            delayMs: () => 11,
        });
    ...
    it("logs catalog and per-product progress", async () => {
        ...
        expect(mockLogger.info).toHaveBeenCalledWith(expect.objectContaining({ listed: 2, catalogComplete: true }), "grabo sku catalog collected");
        ...
        expect(mockLogger.info).toHaveBeenCalledWith(expect.objectContaining({ listed: 2 }), "grabo sku absent-pass start");
        expect(mockLogger.info).toHaveBeenCalledWith(expect.objectContaining({ listed: 2 }), "grabo sku sync finished");
    });
});
