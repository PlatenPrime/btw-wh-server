...
async function seed(productId, url, isOnSite) {
...
describe("markGraboSkusOffSiteUtil", () => {
    ...
    it("sets isOnSite false only for urls not in listed set", async () => {
        ...
        expect((await GraboSku.findOne({ productId: "G1" }).lean())?.isOnSite).toBe(true);
        expect((await GraboSku.findOne({ productId: "G2" }).lean())?.isOnSite).toBe(false);
    });
});
