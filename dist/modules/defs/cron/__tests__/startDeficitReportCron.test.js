...
import { clearDeficitReportLockForTests, tryAcquireDeficitReport, } from "../../utils/deficitReportLock.js";
import { DEFICIT_REPORT_CRON_EXPR, DEFICIT_REPORT_CRON_TZ, startDeficitReportCron, } from "../startDeficitReportCron.js";
describe("startDeficitReportCron", () => {
    ...
    it("creates CronJob weekdays 09:20-17:20 Kyiv", () => {
        ...
        expect(mockedCronJob).toHaveBeenCalledWith(DEFICIT_REPORT_CRON_EXPR, expect.any(Function), null, true, DEFICIT_REPORT_CRON_TZ);
    ...
    it("runs report util on cron tick", async () => {
        ...
        expect(mockLogger.info).toHaveBeenCalledWith({ total: 3 }, "deficit report completed");
    });
    it("skips tick when lock is held", async () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith("deficit report already running, skip cron tick");
    });
    it("logs error and releases lock when runner throws", async () => {
        vi.mocked(runDeficitTelegramReportUtil).mockRejectedValue(new Error("cron boom"));
        startDeficitReportCron();
        await cronCallback?.();
        expect(mockLogger.error).toHaveBeenCalledWith({ err: expect.any(Error) }, "deficit report cron failed");
