...
export const sendDefCalculationStartNotification = async () => {
    try {
        await sendMessageToPlaten("🔄 <b>Начало расчета дефицитов</b>\n\nРасчет дефицитов запущен...");
    }
    catch (error) {
...
export const sendDefCalculationCompleteNotification = async (result) => {
    try {
        ...
        let message = `✅ <b>Расчет дефицитов завершен</b>\n\n` +
            `📊 <b>Результаты:</b>\n` +
        ...
        if (totalDeficits === 0) {
            ...
            const deficitList = Object.entries(result)
                ...
                const status = difQuant <= 0 ? "🔴 Дефицит" : "🟡 Приближение к лимиту";
...
export const sendDefCalculationErrorNotification = async (error) => {
    try {
        const errorMessage = `❌ <b>Ошибка при расчете дефицитов</b>\n\n` +
            `Ошибка: <code>${error instanceof Error ? error.message : "Unknown error"}</code>`;
        await sendMessageToPlaten(errorMessage);
    }
    catch (telegramError) {
        console.error("Failed to send error notification to Platen:", telegramError);
        // Не выбрасываем ошибку, чтобы не прерывать основной процесс
    }
};
