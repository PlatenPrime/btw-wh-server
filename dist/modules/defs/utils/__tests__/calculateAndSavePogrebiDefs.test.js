...
describe("calculateAndSavePogrebiDefs", () => {
    ...
    it("должна корректно выполнять полный цикл расчета и сохранения", async () => {
        ...
        expect(updateCalculationProgress).toHaveBeenCalledWith(1, 4, "Получение лимитов артикулов...");
        expect(getArtLimits).toHaveBeenCalledWith(["ART001", "ART002"]);
        expect(updateCalculationProgress).toHaveBeenCalledWith(2, 4, "Обработка данных Sharik...");
        expect(getSharikStocksWithProgress).toHaveBeenCalledWith(mockPogrebiStocks, mockLimits);
        expect(updateCalculationProgress).toHaveBeenCalledWith(3, 4, "Фильтрация дефицитов...");
        expect(filterDeficits).toHaveBeenCalledWith(mockSharikData);
        expect(updateCalculationProgress).toHaveBeenCalledWith(4, 4, "Сохранение в базу данных...");
    ...
    it("должна обрабатывать ошибки и завершать отслеживание", async () => {
        ...
        await expect(calculateAndSavePogrebiDefs()).rejects.toThrow("Failed to calculate and save pogrebi deficits: Database connection failed");
        expect(sendDefCalculationStartNotification).toHaveBeenCalledTimes(1);
        expect(finishCalculationTracking).toHaveBeenCalledTimes(1);
        expect(sendDefCalculationErrorNotification).toHaveBeenCalledWith(error);
        expect(consoleSpy).toHaveBeenCalledWith("Error in calculateAndSavePogrebiDefs:", error);
        consoleSpy.mockRestore();
    });
    it("должна обрабатывать ошибки без message", async () => {
        const consoleSpy = vi.spyOn(console, "error").mockImplementation(() => { });
        getPogrebiDefStocks.mockRejectedValue("String error");
        await expect(calculateAndSavePogrebiDefs()).rejects.toThrow("Failed to calculate and save pogrebi deficits: Unknown error");
        expect(finishCalculationTracking).toHaveBeenCalledTimes(1);
    ...
    it("должна обрабатывать ошибки на этапе получения лимитов", async () => {
        ...
        getPogrebiDefStocks.mockResolvedValue(mockPogrebiStocks);
        getArtLimits.mockRejectedValue(error);
        await expect(calculateAndSavePogrebiDefs()).rejects.toThrow("Failed to calculate and save pogrebi deficits: Art limits failed");
    ...
    it("должна обрабатывать ошибки на этапе обработки Sharik данных", async () => {
        ...
        getPogrebiDefStocks.mockResolvedValue(mockPogrebiStocks);
        getArtLimits.mockResolvedValue(mockLimits);
        getSharikStocksWithProgress.mockRejectedValue(error);
        await expect(calculateAndSavePogrebiDefs()).rejects.toThrow("Failed to calculate and save pogrebi deficits: Sharik processing failed");
        expect(finishCalculationTracking).toHaveBeenCalledTimes(1);
        expect(sendDefCalculationErrorNotification).toHaveBeenCalledWith(error);
    });
    it("должна обрабатывать ошибки на этапе фильтрации", async () => {
        ...
        getArtLimits.mockResolvedValue(mockLimits);
        getSharikStocksWithProgress.mockResolvedValue(mockSharikData);
        filterDeficits.mockImplementation(() => {
            throw error;
        });
        await expect(calculateAndSavePogrebiDefs()).rejects.toThrow("Failed to calculate and save pogrebi deficits: Filter failed");
        expect(finishCalculationTracking).toHaveBeenCalledTimes(1);
        expect(sendDefCalculationErrorNotification).toHaveBeenCalledWith(error);
    });
