...
describe("calculatePogrebiDefs", () => {
    ...
    it("должна корректно вызывать все функции в правильном порядке", async () => {
        ...
        getSharikStocks.mockResolvedValue(mockSharikData);
    ...
    it("должна передавать ошибки от зависимостей", async () => {
        ...
        await expect(calculatePogrebiDefs()).rejects.toThrow("Database connection failed");
    });
    it("должна обрабатывать ошибки от getArtLimits", async () => {
        ...
        await expect(calculatePogrebiDefs()).rejects.toThrow("Art limits query failed");
    });
    it("должна обрабатывать ошибки от getSharikStocks", async () => {
        ...
        getPogrebiDefStocks.mockResolvedValue(mockPogrebiStocks);
        getArtLimits.mockResolvedValue(mockLimits);
        getSharikStocks.mockRejectedValue(error);
    ...
    it("должна обрабатывать ошибки от filterDeficits", async () => {
        ...
        await expect(calculatePogrebiDefs()).rejects.toThrow("Filter processing failed");
    });
});
