 ...
 */
export const processPullPositionController = async (req, res) => {
    ...
    try {
        ...
        let solver = null;
        let ask = null;
        let completedAsk = null;
        let askProgress = 0;
        let askFullyProcessed = false;
        await session.withTransaction(async () => {
            ...
            if (updatedAsk.quant && updatedAsk.quant > 0 && askProgress > updatedAsk.quant) {
                ...
                console.warn(`Ask ${ask._id} has been over-fulfilled: ${askProgress} > ${updatedAsk.quant}`);
            }
            // 11. Auto-complete ask if fully processed
            if (askFullyProcessed) {
                completedAsk = await completeAskFromPullUtil(solver, solverObjectId, updatedAsk, session);
            }
        ...
        const remainingAsksInPull = await calculateRemainingAsksInPull(palletObjectId, askObjectId);
 ...
 */
async function calculateRemainingAsksInPull(palletId, currentAskId) {
    try {
        const { calculatePullByPalletIdUtil } = await import("../../utils/calculatePullsUtil.js");
        const pull = await calculatePullByPalletIdUtil(palletId);
        if (!pull) {
            return 0;
        }
        // Count unique asks, excluding the current one if it's completed
        const uniqueAskIds = new Set(pull.positions
