...
const KONK = "air";
const PROD = "gemar";
async function createSku(productId, prodName = PROD) {
...
async function createSkugr(title, skuIds, prodName = PROD) {
...
describe("resolveKonkProdSkus", () => {
    ...
    it("with skugrIds returns SKUs grouped by passed order, dedup by productId тАФ first wins", async () => {
        const a = await createSku("a");
        const b = await createSku("b");
