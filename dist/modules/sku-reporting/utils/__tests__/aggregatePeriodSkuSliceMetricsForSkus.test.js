...
describe("aggregatePeriodSkuSliceMetricsForSkus", () => {
    ...
    it("returns ok:false for empty sku list", async () => {
        const r = await aggregatePeriodSkuSliceMetricsForSkus([], new Date("2026-04-02T00:00:00.000Z"), new Date("2026-04-03T00:00:00.000Z"));
    ...
    it("forces sales and revenue to zero on recount days", async () => {
        ...
        const r = await aggregatePeriodSkuSliceMetricsForSkus([{ konkName: "k-recount", productId: "p-1" }], d1, d2);
    ...
    it("keeps zero-sales sku rows", async () => {
        ...
        const r = await aggregatePeriodSkuSliceMetricsForSkus([{ konkName: "k", productId: "p-flat" }], d1, d1);
        expect(r.ok).toBe(true);
        if (!r.ok)
            return;
