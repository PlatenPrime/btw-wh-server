...
describe("updateAskById Controller", () => {
    ...
    it("should update ask with valid data and add action", async () => {
        ...
        expect(responseJson.actions[1]).toBe("15.01.2024 10:30 Solver User: почав обробку запиту");
    });
    ...
    it("should return 400 for invalid status", async () => {
        ...
        // Assert
        expect(responseStatus.code).toBe(400);
        expect(responseJson.message).toBe("Invalid status. Must be one of: new, in_progress, completed, cancelled");
    });
    it("should handle ask with existing actions and add new action", async () => {
        ...
        expect(responseJson.actions).toHaveLength(2);
        expect(responseJson.actions[0]).toBe("15.01.2024 10:00 Asker User: створено запит");
        expect(responseJson.actions[1]).toBe("15.01.2024 10:30 Solver User: додав нову дію");
    });
    ...
    it("should handle ask with empty actions array", async () => {
        ...
        expect(responseJson.actions[0]).toBe("15.01.2024 10:30 Solver User: перша дія для пустого масиву");
    });
