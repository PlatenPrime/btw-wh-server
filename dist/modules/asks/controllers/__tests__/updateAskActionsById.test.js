...
describe("updateAskActionsById Controller", () => {
    ...
    it("should add new action to ask with valid action and userId", async () => {
        ...
        // Act
        await updateAskActionsById(mockRequest, res);
        // Assert
        ...
        expect(responseJson.actions[1]).toBe("15.01.2024 10:30 Action User: додав коментар");
    });
    it("should add multiple actions to ask", async () => {
        ...
        expect(responseStatus.code).toBe(200);
        expect(responseJson.actions).toHaveLength(2);
        expect(responseJson.actions[1]).toBe("15.01.2024 10:30 Action User: перша дія");
        // Add second action
        mockRequest = {
            params: { id: testAsk._id },
            body: {
        ...
        expect(responseJson.actions[2]).toBe("15.01.2024 10:30 Action User: друга дія");
    });
    ...
    it("should handle special characters in action", async () => {
        ...
        expect(responseStatus.code).toBe(200);
        expect(responseJson.actions).toHaveLength(2);
        expect(responseJson.actions[1]).toBe("15.01.2024 10:30 Action User: додав коментар: 'спеціальні символи!@#$%^&*()'");
    });
    it("should handle long action text", async () => {
        ...
        expect(responseStatus.code).toBe(200);
        expect(responseJson.actions).toHaveLength(2);
        expect(responseJson.actions[1]).toBe(`15.01.2024 10:30 Action User: ${longAction}`);
    });
    ...
    it("should handle database error gracefully", async () => {
        ...
        vi.spyOn(Ask, "findByIdAndUpdate").mockRejectedValueOnce(new Error("Database connection failed"));
        mockRequest = {
            params: { id: testAsk._id },
            body: {
                action: "додав коментар",
