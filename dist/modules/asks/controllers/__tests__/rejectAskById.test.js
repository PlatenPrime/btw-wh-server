...
import { rejectAskById } from "../rejectAskById.js";
...
describe("rejectAskById Controller", () => {
    let mockRequest;
    let responseJson;
    let responseStatus;
    let res;
    ...
    it("should reject ask with valid solverId and add rejection action", async () => {
        // Arrange
        mockRequest = {
        ...
        // Act
        await rejectAskById(mockRequest, res);
        // Assert
        ...
        expect(responseJson.actions[0]).toBe("15.01.2024 10:00 Asker User: створено запит");
        expect(responseJson.actions[1]).toBe("15.01.2024 10:30 Solver User: ВІДХИЛИВ запит");
    });
    ...
    it("should handle ask with existing actions and add rejection action", async () => {
    ...
    it("should handle ask with empty actions array", async () => {
        ...
        expect(responseJson.actions[0]).toBe("15.01.2024 10:30 Solver User: ВІДХИЛИВ запит");
    });
    it("should preserve all existing ask fields during rejection", async () => {
        // Arrange
        mockRequest = {
            params: { id: testAsk._id.toString() },
            body: {
        ...
        expect(responseJson.updatedAt.getTime()).toBeGreaterThan(testAsk.updatedAt.getTime());
    });
    ...
    it("should handle ask with multiple existing actions", async () => {
        // Arrange - create ask with multiple actions
        const multiActionAsk = await createTestAsk({
            artikul: "MULTI123",
            nameukr: "Multi Action Ask",
            asker: testUser._id,
            askerData: {
                _id: testUser._id,
                fullname: testUser.fullname,
            ...
            actions: [
                ...
                "15.01.2024 10:15 Solver User: почав обробку",
                "15.01.2024 10:20 Solver User: додав коментар",
            ],
        });
        mockRequest = {
            params: { id: multiActionAsk._id.toString() },
            body: {
                solverId: solverUser._id,
            },
        };
        ...
        // Assert
        expect(responseStatus.code).toBe(200);
        expect(responseJson.actions).toHaveLength(4);
        expect(responseJson.actions[0]).toBe("15.01.2024 10:00 Asker User: створено запит");
        expect(responseJson.actions[1]).toBe("15.01.2024 10:15 Solver User: почав обробку");
        expect(responseJson.actions[2]).toBe("15.01.2024 10:20 Solver User: додав коментар");
        expect(responseJson.actions[3]).toBe("15.01.2024 10:30 Solver User: ВІДХИЛИВ запит");
    });
    it("should always set status to rejected regardless of current status", async () => {
        // Test with different initial statuses
        const statuses = ["new", "completed"];
        for (const status of statuses) {
            // Arrange - create ask with specific status
            const statusAsk = await createTestAsk({
                artikul: `STATUS${status.toUpperCase()}`,
                nameukr: `Status ${status} Ask`,
                asker: testUser._id,
                askerData: {
                    _id: testUser._id,
                    fullname: testUser.fullname,
            ...
            mockRequest = {
                params: { id: statusAsk._id.toString() },
                body: {
                    solverId: solverUser._id,
                },
            };
