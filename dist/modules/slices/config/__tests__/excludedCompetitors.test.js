...
import { excludedCompetitors, getExcludedCompetitorSet, normalizeCompetitorName, } from "../excludedCompetitors.js";
...
describe("getExcludedCompetitorSet", () => {
    ...
    it("reflects excludedCompetitors config", () => {
        for (const sliceType of ["analogSlices", "skuSlices"]) {
            const expected = new Set(excludedCompetitors[sliceType].map((name) => normalizeCompetitorName(name)));
