...
const PAGE2_URL = "https://sharte.example.test/catalog/group/apply/?PAGEN_1=2";
...
function shartePagination(opts) {
    const nextLi = opts.nextHref != null
        ...
        const html = shartePageHtml({
            cards: [
                sharteProductCard({
                    ...
                    title: "Z",
                }).replace(`src="/lazy/lazy-image.svg"`, `src="/lazy/lazy-image.svg" data-srcset="${real} 100w"`),
            ],
        });
        vi.mocked(browserGet).mockResolvedValue(html);
        const result = await getSharteGroupPagesProducts({
