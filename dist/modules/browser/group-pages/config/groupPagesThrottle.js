 ...
 */
export function getGroupPagesThrottleDelayMs(minDelayMs = GROUP_PAGES_THROTTLE_MIN_DELAY_MS, maxDelayMs = GROUP_PAGES_THROTTLE_MAX_DELAY_MS) {
    const min = Math.trunc(minDelayMs);
    const max = Math.trunc(maxDelayMs);
    if (min <= 0 || max <= 0 || min > max) {
        throw new Error(`Invalid group pages throttle range: min=${minDelayMs}, max=${maxDelayMs}`);
    }
