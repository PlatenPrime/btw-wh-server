 ...
 */
export function summarizePlaywrightErrorBody(html) {
...
function readGotoStatus(response) {
    if (response &&
...
async function throwIfHttpError(page, url, status) {
    ...
    const snippetPart = bodySummary.snippet
        ...
        : "";
    throw new Error(`Playwright GET HTTP ${status}: ${url}${titlePart}${snippetPart}`);
}
/**
