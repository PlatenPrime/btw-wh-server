...
import { acquirePlaywrightContext, applyPlaywrightStealth, buildChromiumLaunchOptions, buildPlaywrightContextOptions, closePlaywrightBrowser, resolvePlaywrightHeadless, setPlaywrightChromiumLoaderForTests, withPlaywrightSlot, } from "../playwrightBrowser.js";
...
describe("playwrightBrowser", () => {
    ...
    it("acquirePlaywrightContext без proxy — shared context", async () => {
        ...
        expect(mock.launch).toHaveBeenCalledWith(expect.objectContaining({
            headless: true,
        ...
        expect(mock.newContext).toHaveBeenCalledWith(expect.objectContaining({
            locale: "uk-UA",
            timezoneId: "Europe/Kyiv",
            viewport: { width: 1920, height: 1080 },
        }));
        const firstCall = mock.newContext.mock.calls[0];
    ...
    it("acquirePlaywrightContext с невалидным proxy — throw", async () => {
        const mock = createMockBrowser();
        setPlaywrightChromiumLoaderForTests(async () => ({ launch: mock.launch }));
        await expect(acquirePlaywrightContext({ proxyUrl: "not-a-url" })).rejects.toThrow(/Invalid browser HTTP proxy URL/);
