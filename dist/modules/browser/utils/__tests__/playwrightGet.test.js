...
import { closePlaywrightBrowser, setPlaywrightChromiumLoaderForTests, } from "../playwrightBrowser.js";
import { PLAYWRIGHT_ERROR_BODY_SNIPPET_MAX, playwrightGet, summarizePlaywrightErrorBody, } from "../playwrightGet.js";
describe("summarizePlaywrightErrorBody", () => {
    it("достаёт title и режет snippet", () => {
        ...
        expect(out.snippet.length).toBeLessThanOrEqual(PLAYWRIGHT_ERROR_BODY_SNIPPET_MAX);
...
describe("playwrightGet", () => {
    ...
    it("при HTTP 429 — читает body, логирует snippet и throw", async () => {
        const pageClose = vi.fn(async () => undefined);
        const content = vi.fn(async () => "<html><head><title>Too Many Requests</title></head><body>rate limited by stormwall</body></html>");
        const newPage = vi.fn(async () => ({
        ...
        await expect(playwrightGet(url)).rejects.toThrow(/Playwright GET HTTP 429: .*title="Too Many Requests".*body=.*stormwall/);
    ...
    it("warmUpUrl — сначала origin, потом product в одной вкладке", async () => {
        ...
        const html = await playwrightGet("https://airballoons.com.ua/ua/product/x", { warmUpUrl: "https://airballoons.com.ua/" });
        expect(html).toBe("<html>product</html>");
        expect(goto).toHaveBeenNthCalledWith(1, "https://airballoons.com.ua/", {
            timeout: 30_000,
            waitUntil: "domcontentloaded",
        });
        expect(goto).toHaveBeenNthCalledWith(2, "https://airballoons.com.ua/ua/product/x", {
            timeout: 30_000,
            waitUntil: "domcontentloaded",
        });
    });
    ...
    it("при ошибке goto — Error с коротким message", async () => {
        ...
        await expect(playwrightGet("https://example.com/fail")).rejects.toThrow(/Playwright GET https:\/\/example.com\/fail failed: Navigation timeout/);
        expect(pageClose).toHaveBeenCalled();
    });
