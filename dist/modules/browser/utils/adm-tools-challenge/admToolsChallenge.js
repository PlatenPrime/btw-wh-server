...
const ACK_FORM_APPEND_RE = /form\.append\(\s*['"]___ack['"]\s*,\s*eval\(\s*['"]([^'"]+)['"]\s*\)\s*\)/i;
/** Current JSON body `{__ack: eval('...')}`. */
const ACK_JSON_STRINGIFY_RE = /JSON\.stringify\(\s*\{\s*__ack\s*:\s*eval\(\s*['"]([^'"]+)['"]\s*\)\s*\}\s*\)/i;
 ...
 */
export function isAdmToolsChallengeHtml(html) {
    if (!html) {
        return false;
    }
    const hasAck = html.includes("__ack");
    const hasVendor = html.includes("adm.tools") || html.includes("Захищена сторінка");
 ...
 */
export function detectAdmToolsAckProtocol(html) {
 ...
 */
export function extractAdmToolsAckExpression(html) {
 ...
 */
export function evalAdmToolsAckExpression(expression) {
    const trimmed = expression.trim();
    if (!trimmed || !SAFE_ARITH_RE.test(trimmed)) {
        throw new Error(`Unsafe or empty adm.tools ack expression: ${JSON.stringify(expression)}`);
    }
    let result;
    try {
    ...
    if (typeof result !== "number" || !Number.isFinite(result)) {
        throw new Error(`adm.tools ack did not evaluate to a finite number: ${String(result)}`);
    }
    return result;
}
/**
 ...
 */
export function resolveAdmToolsAckFromHtml(html) {
