 ...
 */
const ACK_APPEND_RE = /form\.append\(\s*['"]___ack['"]\s*,\s*eval\(\s*['"]([^'"]+)['"]\s*\)\s*\)/i;
...
export function evalAdmToolsAckExpression(expression) {
    ...
    if (!trimmed || !SAFE_ARITH_RE.test(trimmed)) {
        throw new Error(`Unsafe or empty adm.tools ___ack expression: ${JSON.stringify(expression)}`);
    }
    let result;
    try {
        ...
        throw new Error(`Failed to eval adm.tools ___ack expression: ${msg}`);
    }
    if (typeof result !== "number" || !Number.isFinite(result)) {
        throw new Error(`adm.tools ___ack did not evaluate to a finite number: ${String(result)}`);
    }
    return result;
}
/**
