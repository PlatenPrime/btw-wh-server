...
function pickImageUrl($img, currentPageUrl) {
    const full = $img.attr("data-full-size-image-url")?.trim() ??
 ...
 */
export function parsePerfectGroupListingProducts($, currentPageUrl) {
    const result = new Map();
    $("article.product-miniature.js-product-miniature[data-id-product]").each((_, el) => {
        const $card = $(el);
        ...
        const rawTitle = $img.attr("alt")?.trim() ||
            $img.attr("title")?.trim() ||
            "";
