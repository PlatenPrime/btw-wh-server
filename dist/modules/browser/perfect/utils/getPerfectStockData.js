...
import { buildPerfectAddToCartBody, extractProductAttributeId, extractProductGroupSelections, extractProductId, extractTitle, extractToken, } from "./perfect-product-page-extract/perfectProductPageExtract.js";
...
function logUnavailableOutcome(productUrl, reason, htmlStatus, hasProductPageHtml) {
    logBrowserError(`Perfect stock unavailable (${reason})`, new Error(`url=${productUrl} status=${htmlStatus} productPageHtml=${hasProductPageHtml}`));
}
function resolveCartFailure(html, pageTitle) {
    return (tryPerfectHtmlFallback(html, pageTitle) ??
...
export async function getPerfectStockData(link) {
    ...
    try {
        ...
        const htmlResponse = await client.get(productUrl, BROWSER_TEXT_CONFIG);
        const html = String(htmlResponse.data ?? "");
        const htmlStatus = htmlResponse.status ?? 0;
        ...
        const cartResp = await client.post(PERFECT_CART_URL, formData, {
            ...BROWSER_TEXT_CONFIG,
            headers: {
        ...
        const cartStatus = cartResp.status ?? 0;
        if (cartStatus >= 400) {
