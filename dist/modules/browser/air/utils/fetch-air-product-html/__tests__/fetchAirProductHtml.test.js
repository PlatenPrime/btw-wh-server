...
import { AIR_PRODUCT_FETCH_RETRY_DELAYS_MS, fetchAirProductHtml, resolveAirProxyAgents, resolveAirProductOrigin, resolveRetryAfterMs, } from "../fetchAirProductHtml.js";
...
describe("resolveAirProductOrigin", () => {
    it("берёт origin из product URL", () => {
        expect(resolveAirProductOrigin("https://airballoons.com.ua/ua/product/shar-assorti")).toBe("https://airballoons.com.ua");
    });
});
describe("resolveAirProxyAgents", () => {
    ...
    it("бросает при невалидном proxy URL", () => {
        ...
        expect(() => resolveAirProxyAgents()).toThrow(/Invalid browser HTTP proxy URL/);
    });
});
describe("fetchAirProductHtml", () => {
    const PRODUCT = "https://airballoons.com.ua/ua/product/shar-assorti-neonovyj-10-26sm";
    ...
    it("бросает при пустом url", async () => {
        await expect(fetchAirProductHtml("")).rejects.toThrow("Product URL is required and must be a string");
        await expect(fetchAirProductHtml("   ")).rejects.toThrow("Product URL is required and must be a string");
    });
    it("бросает при невалидном url", async () => {
        await expect(fetchAirProductHtml("not-a-url")).rejects.toThrow(/Invalid air product URL/);
    });
    it("бросает до запросов при невалидном proxy URL", async () => {
        process.env.AIR_HTTP_PROXY_URL = "socks5://user:pass@10.0.0.1:50101";
        await expect(fetchAirProductHtml(PRODUCT)).rejects.toThrow(/Invalid browser HTTP proxy URL/);
    ...
    it("warm-up → product с Cookie и Referer", async () => {
        ...
        expect(mockGet.mock.calls[1][1].headers["User-Agent"]).toContain("Chrome/131");
    ...
    it("при 429 без Retry-After использует fallback delay", async () => {
        ...
        await fetchAirProductHtml(PRODUCT);
        expect(mockSleep).toHaveBeenCalledWith(AIR_PRODUCT_FETCH_RETRY_DELAYS_MS[0]);
    });
    it("после трёх 429 бросает HTTP 429", async () => {
        mockGet
            ...
            .mockResolvedValueOnce({ status: 429, data: "", headers: {} })
            .mockResolvedValueOnce({ status: 429, data: "", headers: {} });
        await expect(fetchAirProductHtml(PRODUCT)).rejects.toThrow(`Browser GET HTTP 429: ${PRODUCT}`);
        expect(mockSleep).toHaveBeenCalledTimes(2);
        expect(mockSleep).toHaveBeenNthCalledWith(1, AIR_PRODUCT_FETCH_RETRY_DELAYS_MS[0]);
        expect(mockSleep).toHaveBeenNthCalledWith(2, AIR_PRODUCT_FETCH_RETRY_DELAYS_MS[1]);
    });
    ...
    it("бросает при пустом body на 200", async () => {
        ...
        await expect(fetchAirProductHtml(PRODUCT)).rejects.toThrow(`Browser GET empty body: ${PRODUCT}`);
    });
    it("сразу бросает на non-429 ошибке", async () => {
        ...
        await expect(fetchAirProductHtml(PRODUCT)).rejects.toThrow(`Browser GET HTTP 503: ${PRODUCT}`);
        expect(mockSleep).not.toHaveBeenCalled();
    });
});
