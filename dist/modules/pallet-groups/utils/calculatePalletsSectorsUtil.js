 ...
 */
export const calculatePalletsSectorsUtil = async (options = {}) => {
    ...
    const palletUpdatesMap = new Map();
    const palletOperations = [];
    const palletsInGroups = new Set();
    ...
    if (!groupIds) {
        const palletsWithoutGroup = allPallets.filter((pallet) => !palletsInGroups.has(pallet._id.toString()));
