...
describe("movePalletPosesController", () => {
    ...
    it("200: перемещает poses из source в target паллету", async () => {
        ...
        const updatedSource = await PalletModel.findByIdAndUpdate(sourcePallet._id, { $set: { poses: [pos._id] } }, { new: true });
    ...
    it("400: если source и target ID одинаковые", async () => {
        ...
        expect(responseJson.message).toBe("Source and target pallet IDs must be different");
    });
