...
import { getAllRoles, getAllUsers, getMe, getUserById, login, registrateUser, updateUserInfo, } from "./controllers/index.js";
const router = Router();
...
router.get("/users/:id", checkAuth, checkRoles([RoleType.USER]), async (req, res) => {
    try {
...
router.get("/me/:id", checkAuth, checkRoles([RoleType.USER]), async (req, res) => {
    try {
        await getMe(req, res);
    }
    catch (error) {
...
router.put("/users/:userId", checkAuth, checkRoles([RoleType.ADMIN]), async (req, res) => {
    try {
