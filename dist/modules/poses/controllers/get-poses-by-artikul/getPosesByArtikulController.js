...
export const getPosesByArtikulController = async (req, res) => {
    try {
        ...
        const { pogrebi: pogrebiPoses, merezhi: merezhiPoses } = groupPosesByWarehouseUtil(poses);
        ...
        const totalQuant = poses.reduce((sum, pos) => sum + pos.quant, 0);
        const totalBoxes = poses.reduce((sum, pos) => sum + pos.boxes, 0);
        const response = {
            total: poses.length,
            pogrebi,
            merezhi,
