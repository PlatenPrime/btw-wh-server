...
describe("updateAllPosesNameukr", () => {
    ...
    it("should update all Pos documents with nameukr field", async () => {
        ...
        mockPos.find.mockResolvedValue(mockPoses);
        // Mock Art.findOne for each artikul
        mockArt.findOne
            .mockResolvedValueOnce(mockArts[0]) // ART001
            .mockResolvedValueOnce(mockArts[1]) // ART002
            .mockResolvedValueOnce(null); // ART003 not found
        // Mock Pos.updateOne for each update
        mockPos.updateOne.mockResolvedValue({ modifiedCount: 1 });
        await updateAllPosesNameukr(mockRequest, mockResponse);
        ...
        expect(mockPos.updateOne).toHaveBeenCalledWith({ _id: "pos1" }, { $set: { nameukr: "Товар 1" } });
        expect(mockPos.updateOne).toHaveBeenCalledWith({ _id: "pos2" }, { $set: { nameukr: "Товар 2" } });
        expect(mockPos.updateOne).toHaveBeenCalledWith({ _id: "pos3" }, { $set: { nameukr: "" } });
    });
