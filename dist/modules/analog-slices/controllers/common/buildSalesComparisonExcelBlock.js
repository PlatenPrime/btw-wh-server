 ...
 */
export function setupSalesComparisonHeaderRow(params) {
    const { worksheet, items, dataStartCol, totalCol, diffSalesCol, diffSalesPctCol, diffRevenueCol, diffRevenuePctCol, columnCount, } = params;
...
export function buildSalesComparisonExcelBlock(options) {
    ...
    if (Number.isFinite(diffSalesPieces) && diffSalesPieces !== 0) {
        diffSalesCell.font = {
            ...(diffSalesCell.font ?? {}),
            color: diffSalesPieces > 0 ? { argb: "FF00AA00" } : { argb: DELIVERY_DAY_RED },
        };
    }
    ...
    if (totalAnalogSales === 0) {
        ...
        if (rounded !== 0) {
            diffSalesPctCell.font = {
                ...(diffSalesPctCell.font ?? {}),
                color: rounded > 0 ? { argb: "FF00AA00" } : { argb: DELIVERY_DAY_RED },
            };
        }
    }
 ...
 */
export function buildSalesComparisonSummaryBlock(options) {
    const { worksheet, startRow, keyCol, valueCol, totalAnalogSales, totalAnalogRevenue, totalBtradeSales, totalBtradeRevenue, } = options;
    const diffSalesPieces = totalBtradeSales - totalAnalogSales;
    const diffRevenueUah = totalBtradeRevenue - totalAnalogRevenue;
    const diffSalesPct = totalAnalogSales === 0
        ? null
        : (totalBtradeSales / totalAnalogSales - 1) * 100;
    const diffRevenuePct = totalAnalogRevenue === 0
        ? null
    ...
    entries.forEach(([key, value], index) => {
        ...
        if (value !== null && (key.startsWith("Δ Продажі, %") || key.startsWith("Δ Виручка, %"))) {
            ...
            row.getCell(valueCol).value = value / 100;
        }
