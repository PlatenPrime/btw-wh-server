...
export function buildAnalogBtradeExcelBlock(options) {
    ...
    const metrics = [
        { row: analogStockRow, getVal: (item) => item.analogStock },
        { row: analogPriceRow, getVal: (item) => item.analogPrice },
        { row: btradeStockRow, getVal: (item) => item.btradeStock },
        { row: btradePriceRow, getVal: (item) => item.btradePrice },
    ];
    for (const { row, getVal } of metrics) {
        ...
        const firstVal = typeof firstRaw === "number" ? firstRaw : Number(firstRaw ?? NaN);
        const lastVal = typeof lastRaw === "number" ? lastRaw : Number(lastRaw ?? NaN);
    ...
    if (items.length > 1) {
        const getFirstAndLastNumeric = (selector) => {
            let first = null;
            let last = null;
        ...
        if (analogBounds && btradeBounds) {
            ...
            if (Number.isFinite(deltaAnalog) && Number.isFinite(deltaBtrade)) {
                ...
                worksheet.mergeCells(startRow, summaryDiffPctCol, startRow + 3, summaryDiffPctCol);
            }
        }
    }
