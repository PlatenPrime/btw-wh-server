...
import { computeArtSalesPointsFromSeries, } from "./loadArtBtradeSliceSeries.js";
const META_HEADERS = ["Артикул", "Назва (укр)", "Показник"];
const SALES_METRICS = ["Продажі", "Ціна", "Виручка"];
function salesPointValue(point, metricIndex) {
    if (metricIndex === 0)
        return point.sales;
    if (metricIndex === 1)
        return point.price;
...
export async function buildArtSalesExcel(input) {
    const salesPoints = computeArtSalesPointsFromSeries(input.datesReport, input.coalescedFull, input.reportIndexStart);
