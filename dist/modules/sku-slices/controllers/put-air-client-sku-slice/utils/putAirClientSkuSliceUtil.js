 ...
 */
export async function putAirClientSkuSliceUtil(input, now = new Date()) {
    ...
    await SkuSlice.findOneAndUpdate({ konkName: AIR_CLIENT_SLICE_KONK, date: sliceDate }, {
        $setOnInsert: {
            konkName: AIR_CLIENT_SLICE_KONK,
            date: sliceDate,
            data: {},
    ...
    }, { $set: { [`data.${productId}`]: dataItem } }, { new: true });
    if (!updated) {
