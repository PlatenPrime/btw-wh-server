...
describe("buildCompensatingDataKeyQueue", () => {
    ...
    const shouldInclude = (item) => typeof item === "object" &&
        item !== null &&
        item.stock === -1;
    ...
    it("handles missing konkName and data", () => {
        const queue = buildCompensatingDataKeyQueue([{ konkName: undefined, data: undefined }], excluded, () => true);
...
describe("runCompensatingSliceRefetchLoop", () => {
    ...
    it("aggregates stats from processItem", async () => {
        ...
        expect(logModuleInfo).toHaveBeenCalledWith("slice-compensation", "compensating slice refetch item start", {
            index: 1,
            total: 2,
            konkName: "air",
            dataKey: "P1",
        });
        expect(logModuleInfo).toHaveBeenCalledWith("slice-compensation", "compensating slice refetch item done", {
            index: 1,
            total: 2,
            konkName: "air",
            dataKey: "P1",
    ...
    it("does not delay for single-item queue", async () => {
        ...
        await runCompensatingSliceRefetchLoop([{ konkName: "air", dataKey: "P1" }], processItem);
