...
import { clearCompensatingRunsForTests, isCompensatingRunActive, tryAcquireCompensatingRun, } from "../compensatingRunStatus.js";
...
describe("runManualCompensatingSlice", () => {
    ...
    it("warns and skips runner when lock already held", async () => {
        ...
        expect(mockLogger.warn).toHaveBeenCalledWith({ konkName: "air" }, "compensating slice already running");
    ...
    it("logs error and releases lock when runner throws", async () => {
        ...
        await runManualCompensatingSlice("air");
        expect(mockLogger.error).toHaveBeenCalledWith({ err, konkName: "air" }, "manual compensating slice failed");
        expect(isCompensatingRunActive("air")).toBe(false);
    });
});
