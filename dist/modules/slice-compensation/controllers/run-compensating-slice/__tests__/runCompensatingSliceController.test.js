...
import { clearCompensatingRunsForTests, tryAcquireCompensatingRun, } from "../../../utils/compensatingRunStatus.js";
...
describe("runCompensatingSliceController", () => {
    ...
    it("400 on validation error", async () => {
        mockReq.body = {};
        await runCompensatingSliceController(mockReq, mockRes);
        expect(mockStatus).toHaveBeenCalledWith(400);
        expect(mockJson).toHaveBeenCalledWith(expect.objectContaining({ message: "Validation error" }));
    ...
    it("500 on util error and releases lock", async () => {
        vi.mocked(runCompensatingSlicesForKonk).mockRejectedValue(new Error("boom"));
        mockReq.body = { konkName: "air" };
        await runCompensatingSliceController(mockReq, mockRes);
