...
describe("getServerEgressGeo", () => {
    ...
    it("возвращает нормализованный geo при успешном ответе ipapi.co", async () => {
        ...
        expect(mockedAxios.get).toHaveBeenCalledWith("https://ipapi.co/json/", expect.objectContaining({ timeout: 5000 }));
    });
    it("использует SERVER_EGRESS_GEO_URL при заданном env", async () => {
        ...
        expect(mockedAxios.get).toHaveBeenCalledWith("https://custom.example/geo", expect.objectContaining({ timeout: 5000 }));
    });
