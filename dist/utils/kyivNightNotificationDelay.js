...
export function isKyivNightHours(d = new Date()) {
 ...
 */
export function getMsUntilKyivMorningSend(d = new Date()) {
    if (!isKyivNightHours(d)) {
        return 0;
    }
    const hour = getKyivHour(d);
    let hoursUntilMorning;
    ...
    const minute = Number(kyivMinuteFormatter.formatToParts(d).find((p) => p.type === "minute")?.value);
    const second = Number(kyivSecondFormatter.formatToParts(d).find((p) => p.type === "second")?.value);
    const msUntilMorning = hoursUntilMorning * 60 * 60 * 1000 -
        minute * 60 * 1000 -
        second * 1000 -
        d.getMilliseconds();
    return Math.max(0, msUntilMorning);
}
async function sendAnalyticsChatNotificationSafe(message) {
    try {
 ...
 */
export async function sendAnalyticsChatNotificationDeferred(message, finishedAt = new Date()) {
